...
EAPI=2
inherit toolchain-funcs multilib versionator eutils prefix
...
src_configure() {
	# Configuration steps...
	echo "PREFIX = \$(dir \$\"\$(EPREFIX)/usr\")" > .config
...
src_install() {
	...
	emake EPREFIX="${EPREFIX}" DESTDIR="${D}" install || die "emake install failed"
	dodoc CHANGELOG.txt
	if use doc; then
